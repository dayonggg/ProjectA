...
<script>
	...
	export default {
		...
		mounted() {
			// console.log('D~~~~', D.checkVersion())
			// D.checkResourceDir(path.join('/Users/yongshi/Documents/table','tttt'))
// 			D.getTablelist(function(result) {
// 				console.log(result)
// 			})
			// D.initTableTree()
			D.init()
		...
		methods: {
			...
			addWS() {
				fs.exists(path.join(this.workSpace.resDir, 'Assets'), e => {
					if (e) {
						localStorage.setItem('tableDir', this.workSpace.tableDir)
						localStorage.setItem('resDir', this.workSpace.resDir)
						this.initProjConfig()
			...
			addScene() {
				if (this.sceneList.indexOf(path.basename(this.newScene.file)) > -1) {
					...
					let imgTarget = path.format({
						dir: path.join(this.workSpace.resDir, 'scenes'),
